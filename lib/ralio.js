...
Ralio.prototype.story = function (storyID, callback) {
  var self = this,
      ...
      query = {
        fetch: 'Name,FormattedID,PlanEstimate,ScheduleState,Tasks,Tags,Pair,Defects,State,Owner,SubmittedBy,TaskIndex,Blocked,Project,ObjectID,Description',
  ...
  this.bulk(query, function (error, data) {
    if (error !== null) {
      ...
      if (options.state) {
        ...
        if (options.state === 'Tested' && type ==  "DE") {
          taskObject.State = "Closed";
          taskObject.Resolution = "Code Change";
        }
      }
