  ...
  this.ralio.story(this.fetchID(story), function (error, story) {
    ...
    if (story === null) {
      ...
      child_process.exec('elinks -dump ' + descTempPath, function (err, stdout, stderr) {
        ...
        if(story.Owner) {
          console.log('Owner: ' + story.Owner._refObjectName);
        }
        if(story.SubmittedBy) {
          console.log('Reporter: ' + story.SubmittedBy._refObjectName);
        }
        console.log('-----');
        fs.unlinkSync(descTempPath);

        (story.Tasks.length > 0) ? 
          self.printStoryLine(story, {owners: true}) : self.printTaskLine(story, {tab: false, points: true});
...
CLI.prototype.transition = function (task, state, options) {
  var self = this,
      update_options = {state: state, pair: options.pair};
